...
setMethod("show", "simumix", function(object){
  ...
  cat("\n@mix.prof: ", nrow(x@mix.prof),"x",ncol(x@mix.prof),"data frame of the genotypes of the mixture contributors")
  cat("\n@mix.all: list of the alleles found in the mixtures")
  cat("\n@popinfo:",  ifelse(is.null(x@popinfo), "- empty -", "populations of the mixture contributors"))
  cat("\n")
} 
)
